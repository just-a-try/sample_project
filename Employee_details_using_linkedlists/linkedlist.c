...
struct employee* add_employee(struct employee* first_employee ) //remove this argument
{
	...
	do
	{
		...
		scanf("%s",employee_ptr->employee_name);
	} while(!stringValidation(employee_ptr->employee_name));
  //strcpy(employee_ptr->employee_name,employee_details);
	// getting employee id
	do
	{
...
struct employee* removeEmployee(struct employee * first_employee,int sorting_type)
{
	char name[NAME_LENGTH];
	int ID;
	if(first_employee)
	{
		struct employee *current=first_employee,*next=current->next_employee,*previous=NULL;
		if(!sorting_type)
		{
			do
			{
		...
		while(1)
		{
			...
			{
				if((!sorting_type)?strcmp(current->employee_name,name)==0 : current->employee_id == ID)
				{
	...
	{
		printf("Add any employee to remove\n");
	}
	return first_employee;
}


struct employee* sort_employee_details(struct employee* first_employee,int sorting_type)//remove employee count argument
{
	int number_of_swap;
	struct employee *current,*next,*previous;
	if(first_employee->next_employee!=NULL)
	{
		do{
			...
			while(next!=NULL)
			{
				if((!sorting_type)?strcmp(current->employee_name,next->employee_name)>0 : current->employee_id > next->employee_id)
					{
						if(current==first_employee)
						{
							first_employee=next;
		...
		}while(number_of_swap>1);

	}
	else
	{
		printf("Add another employee to sort\n");
	}
	return first_employee;
}
    ...
    switch(choice)
		{
			  case ADD:
					...
					{
						printf("add function is not executed \n");
    			}
					break;
				case VIEW:
