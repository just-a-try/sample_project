...
enum menu{ADD=1,VIEW,SORT_BY_NAME,REMOVE_BY_ID,SORT_BY_ID,REMOVE_BY_NAME,EXIT};
bool string_validation(char *employee_name)
{
	...
	while(indexOfchar<string_length)
		{
			if((employee_name[indexOfchar]>='A' && employee_name[indexOfchar]<='Z')||( employee_name[indexOfchar]>='a' && employee_name[indexOfchar]<='z'))
			{
				indexOfchar++;
				continue;
			}
...
struct employee* add_employee(struct employee *employee_ptr,int *employee_count)
{
	...
	{
		employee_ptr=(struct employee *)realloc(employee_ptr,(*employee_count+1)*sizeof(struct employee));
		if(employee_ptr==NULL)
		{
	...
	// Change as discussed
		do {
			...
			scanf("%s",employee_ptr[*employee_count].employee_name);
...
void display_employee_details(struct employee *employee_ptr , int employee_count)
{
 	int index;
...
void sort_byname(struct employee *employee_ptr,int employee_count) //remove the temp argument
{
  ...
  struct employee temp;
	//bubble sort
	for(employee_count_i=0;employee_count_i<employee_count-1;employee_count_i++)
	{

		for(employee_count_j=employee_count_i+1;employee_count_j<=employee_count-1;employee_count_j++)
		{

			while(employee_ptr[employee_count_i].employee_name[char_index]==employee_ptr[employee_count_j].employee_name[char_index])
			{

				if(employee_ptr[employee_count_i].employee_name[char_index]=='\0' && employee_ptr[employee_count_j].employee_name[char_index]=='\0')
				{
					break;
				}
				char_index++;
			...
			if(employee_ptr[employee_count_i].employee_name[char_index]>employee_ptr[employee_count_j].employee_name[char_index])
			{
...
void sort_byID(struct employee *employee_ptr,int employee_count)
{
	...
	for(employee_count_i=0;employee_count_i<employee_count-1;employee_count_i++)
	{
		...
		for(employee_count_j=employee_count_i+1;employee_count_j<=employee_count-1;employee_count_j++)
		{
			...
			if(employee_ptr[employee_count_i].employee_id>employee_ptr[employee_count_j].employee_id)
			{
    ...
    switch(choice)
		{
		  ...
		  case VIEW:
		    	...
		    	{
						display_employee_details(employee_ptr,employee_count);
          }
		      else
					{
				    ...
				    if(remove_employee_byid(employee_ptr,&employee_count))
						{
							...
							{
								employee_ptr=(struct employee *)realloc(employee_ptr,(employee_count)*sizeof(struct employee));//realloc validation and display message accordingly
								if(employee_ptr==NULL)
								{
						...
						{
							printf("removing by id function not executed\n");
						}
		      }
		    	else
		    	{
		    ...
		    case REMOVE_BY_NAME:
			    if(employee_count!=0)
		    	{
				    if(remove_employee_byname(employee_ptr,&employee_count)) //same as remove by id
						...
						{
							printf("removing by name function not executed\n");
						}
	        }
		    	else
		    	{
